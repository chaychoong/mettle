Status: PINNED as recon (mt-070, tech-lead reviewed 2026-07-27 — the `backloop`-vs-`looplength` and no-LICENSE finds re-verified against the clones). **External-tool contract, NOT jar-pinned authority** — the oracle jar ships no Sterling, so nothing here is a conformance surface. **Owner resolution same day ([ADR-0016](../adr/0016-rung5-remainder-serve-xml-packaging.md) Resolutions): mettle writes its own frontend (mt-075) and embeds nothing from the Sterling lineage — the license question is moot for shipped code; §2's wire protocol is what mettle keeps** (mt-072's serve backend speaks it; external Sterling connecting is best-effort interop, which is the only remaining consumer of §9/§10's open cells — probe them only if/when the `backloop` compat adapter is written.)

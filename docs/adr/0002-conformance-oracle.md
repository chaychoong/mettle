...
## Decision
1. **The yardstick is a single pinned reference jar: the latest stable Alloy 6 release.** Its exact version, git tag, and jar SHA-256 are recorded in [reference/alloy6-reference.md](../reference/alloy6-reference.md) and treated as *the* oracle. Any other Alloy version is out of scope for conformance. *(Exact version/SHA being filled in by the oracle-brief task, bead mt-002.)*

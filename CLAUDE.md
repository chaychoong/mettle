...
A bare **"proceed"** from the product owner (or any resume with no specifics) means: **do the "Next chunk" in [docs/STATE.md](docs/STATE.md) — that one chunk, then stop and report.** Don't ask what to work on — STATE.md is authoritative — but also don't chain into the chunk after it: "proceed" authorizes one chunk, not a run (owner feedback, 2026-07-16). Multi-chunk runs happen only when the owner explicitly grants one — and the standing grant format (owner, 2026-07-27) is **"keep going"**, optionally "until N% context": chain Next-chunks until context-window usage reaches **N% (default 30%)**, then finish the chunk in flight, run [docs/SESSION_WRAP.md](docs/SESSION_WRAP.md), and stop. Real usage is injected by a machine-level PostToolUse hook (`~/.claude/hooks/context-usage.sh`, reports at ≥15%); trust its number, never push past the threshold (auto-compact is off on this machine — overshooting degrades context). Concretely, on cold pickup:

...
A bare **"proceed"** from the product owner (or any resume with no specifics) means: **do the "Next chunk" in [docs/STATE.md](docs/STATE.md) — that one chunk, then stop and report.** Don't ask what to work on — STATE.md is authoritative — but also don't chain into the chunk after it: "proceed" authorizes one chunk, not a run (owner feedback, 2026-07-16). Multi-chunk runs happen only when the owner explicitly grants one — and the standing grant format (owner, 2026-07-27; alert-driven form 2026-08-21) is **"keep going"** / **"proceed and keep going"**: chain Next-chunks until the context-usage hook fires its wrap alert, then finish the chunk in flight, run [docs/SESSION_WRAP.md](docs/SESSION_WRAP.md), and stop — no owner check-in needed at the threshold; the alert is the authorization to wrap. The machine-level PostToolUse hook (`~/.claude/hooks/context-usage.sh`) is silent below 300k tokens (30% of the window), then injects a wrap-now alert on every percent gained: silence means keep going, the alert IS the stop signal. Trust its number, never push past it (auto-compact is off on this machine — overshooting degrades context). An explicit "until N% context" from the owner overrides the default: past 30% the alert fires every percent anyway, so keep chaining until the alert's own percentage reaches N, then wrap. Concretely, on cold pickup:
...
- **Gate at rungs; pace by chunks.** Surface to the human at each rung of **[docs/ROADMAP.md](docs/ROADMAP.md)** with a build they can run and one concrete thing to look for. _Standing exception (owner, 2026-07-27): all human tests are batched into ONE combined feature-complete review after the [ADR-0016](docs/adr/0016-rung5-remainder-serve-xml-packaging.md) bead set (mt-071/072/075/076/073/074) — per-rung gating is suspended until that review; genuine forks still surface as they arise._ Within a rung, work one chunk per "proceed" and check in between chunks (see "Start here"); surface genuine decisions (solver choice, scope calls, gates) as questions *before* starting dependent work, not inside a status report afterward.
